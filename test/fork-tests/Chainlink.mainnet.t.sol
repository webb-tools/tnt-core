...
import {
    ChainlinkAdapter,
    LINK
} from "core/lst/adapters/ChainlinkAdapter.sol";
...
address constant VALIDATOR_1 = 0xe7DB0D2384587956ef9d47304E96236022cCE3Af; 
...
contract ChainlinkForkTest is Test, LiquifierEvents, ERC721Receiver {
    ...
    function setUp() public {
        ...
        adapter = new ChainlinkAdapter{ salt: salt }();
        fixture.registry.registerAdapter(address(LINK), address(adapter));
        balance = LINK.balanceOf(TOKEN_HOLDER);
        vm.prank(TOKEN_HOLDER);
        LINK.transfer(address(this), balance);
    }

    function test_registry_AdapterRegistered() public {
        assertEq(fixture.registry.adapter(address(LINK)), address(adapter), "adapter not registered");
    }
